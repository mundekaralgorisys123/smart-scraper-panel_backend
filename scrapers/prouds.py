...
class ProudsScraper:
    """Parser for Prouds.com product pages with database and Excel functionality"""
    ...
    def parse_and_save_products(self, products_data: List[Dict], page_title: str, page_url: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            image_folder = os.path.join(self.image_save_path, f"prouds_{timestamp}")
            os.makedirs(image_folder, exist_ok=True)
            
            # Create Excel file
            excel_filename = f"prouds_scraped_products_{timestamp}.xlsx"
    ...
    def extract_individual_products_from_html(self, html_content: str) -> List[str]:
        """Extract individual product HTML blocks from Prouds HTML"""
        if not html_content:
            return []
        
        soup = BeautifulSoup(html_content, 'html.parser')
        
        # Prouds specific product selectors
        product_selectors = [
        ...
        print(f"Found {len(individual_products)} product tiles in Prouds HTML")
        return individual_products
    
    def _extract_product_name(self, soup) -> str:
        """Extract product name from Prouds product tile"""
    ...
    def _extract_price(self, soup) -> str:
        """Extract price information from Prouds product"""
        try:
            price_now_element = soup.select_one('span.s-price__now')
            price_was_element = soup.select_one('span.s-price__was')
            full_price_element = soup.select_one('div.s-product__price.s-price')
            ...
            if price_now and price_was:
                ...
                return price_now
            elif full_price_element:
                # Only use full_price_element if both price_now and price_was are missing
                price = full_price_element.get_text(strip=True)
                extracted_price = self.extract_price_value(price)
                return extracted_price if extracted_price != "N/A" else price
            else:
                return "N/A"
    ...
    def _extract_image(self, soup) -> str:
        """Extract product image URL from Prouds product"""
        try:
    ...
    def _extract_link(self, soup) -> str:
        """Extract product link from Prouds product"""
        # Link selectors for Prouds
        link_selectors = [
    ...
    def _extract_gold_type(self, product_name: str) -> str:
        ...
        gold_patterns = [
            ...
            r"\b(?:14K|18K|24K)\s+(?:YELLOW|WHITE|ROSE)\s+GOLD\b",
    ...
    def _extract_badges(self, soup) -> list:
        """Extract badge information from Prouds product"""
        badges = []
    ...
    def _extract_promotions(self, soup) -> str:
        """Extract promotion text from Prouds product"""
        # For Prouds, the price comparison itself serves as promotion info
        price_was_element = soup.select_one('span.s-price__was')
    ...
    def _normalize_image_url(self, url: str) -> str:
        """Normalize image URL for Prouds"""
        ...
        if url.startswith('http'):
            ...
            return f"https://www.prouds.com.au{url}"
        return url
    
    def _normalize_link_url(self, url: str) -> str:
        """Normalize link URL for Prouds"""
        if not url or url == "N/A":
            return "N/A"
        if url.startswith('http'):
            return url
