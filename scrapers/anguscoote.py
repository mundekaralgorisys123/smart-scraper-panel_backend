...
class AngusCooteScraper:
    """Parser for AngusCoote.com.au product pages with database and Excel functionality"""
    ...
    def parse_and_save_products(self, products_data: List[Dict], page_title: str, page_url: str = "") -> Dict[str, Any]:
        ...
        """
        try:
            print("=================== Starting Angus & Coote Scraper ==================")
            ...
            image_folder = os.path.join(self.image_save_path, f"anguscoote_{timestamp}")
            os.makedirs(image_folder, exist_ok=True)
            
            # Create Excel file
            excel_filename = f"anguscoote_scraped_products_{timestamp}.xlsx"
            ...
            wb = Workbook()
            sheet = wb.active
            sheet.title = "Angus & Coote Products"
    ...
    def extract_individual_products_from_html(self, html_content: str) -> List[str]:
        """Extract individual product HTML blocks from Angus & Coote HTML"""
        if not html_content:
            return []
        
        soup = BeautifulSoup(html_content, 'html.parser')
        
        # Angus & Coote specific product selectors (same structure as Goldmark/Prouds)
        product_selectors = [
        ...
        print(f"Found {len(individual_products)} product tiles in Angus & Coote HTML")
        return individual_products
    
    def _extract_product_name(self, soup) -> str:
        """Extract product name from Angus & Coote product tile"""
    ...
    def _extract_price(self, soup) -> str:
        """Extract price information from Angus & Coote product"""
        try:
    ...
    def _extract_image(self, soup) -> str:
        """Extract product image URL from Angus & Coote product"""
        try:
    ...
    def _extract_link(self, soup) -> str:
        """Extract product link from Angus & Coote product"""
        # Link selectors for Angus & Coote
        link_selectors = [
    ...
    def _extract_badges(self, soup) -> list:
        """Extract badge information from Angus & Coote product"""
        badges = []
    ...
    def _extract_promotions(self, soup) -> str:
        """Extract promotion text from Angus & Coote product"""
        # For Angus & Coote, the price comparison itself serves as promotion info
        price_was_element = soup.select_one('span.s-price__was')
    ...
    def _normalize_image_url(self, url: str) -> str:
        """Normalize image URL for Angus & Coote"""
        ...
        if url.startswith('http'):
            ...
            return f"https://www.anguscoote.com.au{url}"
        return url
    
    def _normalize_link_url(self, url: str) -> str:
        """Normalize link URL for Angus & Coote"""
        if not url or url == "N/A":
            return "N/A"
        if url.startswith('http'):
            return url
    ...
    def modify_image_url(self, image_url: str) -> str:
        ...
        # Angus & Coote uses width parameter in their image URLs
